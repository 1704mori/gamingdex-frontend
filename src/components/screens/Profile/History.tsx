...
export default function History({ data }: Props) {
  const buildDayColor = (value: number) => {
    ...
    // if (value < 5) return 'bg-[#1890FF]'
    // if (value < 10) return 'bg-[#2F9BFF]'
    // if (value < 15) return 'bg-[#46A6FF]'
    // if (value < 20) return 'bg-[#5DB1FF]'
  ...
  const buildDays = useMemo(() => {
    ...
    return squares;
  }, [data, currentYear, adjustedStartMonth, currentMonth]);
